...
def solve_part_one(lines: list[str]):
    ...
    for y in range(len(trail_map)):
        for x in range(len(trail_map)):
            ...
            pos = (x, y)
            reached = set()
            walk_trail(trail_map, pos, 0, reached)
            sum_total += len(reached)

    return sum_total


def walk_trail(trail_map: list[list[int]], pos: tuple[int, int], value: int,
    ...
    for move_x, move_y in moves:
        ...
        walk_trail(trail_map, next_pos, value, reached, visited)
